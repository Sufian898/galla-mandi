    ...
    if (customerPartyId) {
      try {
        ...
        if (party && party.type === 'supplier') {
          // Get netSale (پختہ بِکری)
          const netSale = bill.netSale || 0;
          
          // Get cash amount (نقدی) from expenses
          let cashAmount = 0;
          if (expenses) {
            const exp = expenses;
            cashAmount = Number(exp.Cash ?? exp['نقدی'] ?? 0) || 0;
          }

          const billSerial = bill.serialNumber || bill._id?.toString().slice(-6) || '';
          const billDescription = 'بل #' + billSerial;

          // 1. Create Sale transaction: پختہ بِکری + نقدی (debit - increases balance)
          const saleAmount = netSale + cashAmount;
          if (saleAmount > 0) {
            const saleTx = new LedgerTransaction({
              companyId,
              partyId: customerPartyId,
              billId: bill._id,
              date: bill.date,
              description: billDescription + ' (سیل)',
              type: 'debit',
              amount: saleAmount
            });
            await saleTx.save();
          ...
          // 2. Create Recovery transaction: نقدی (credit - reduces balance)
          if (cashAmount > 0) {
...
// Helper function to recalculate party balances (same as in ledger.js)
